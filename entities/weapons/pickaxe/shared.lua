...
function SWEP:PrimaryAttack()
	...
	elseif !ply:CanCarry(Tbl.picked,Tbl.pickedam) then ply:LiquidChat("Mining", Color(100,100,100), "You need to free up inventory space before mining this.") return
	...
	timer.Create("Mining_" .. plyid, 1, Picktime, function()
		...
		if SERVER and Picked == Picktime then
			ply.Picking = false
			if math.random(1,Tbl.minechance) == 1 then
